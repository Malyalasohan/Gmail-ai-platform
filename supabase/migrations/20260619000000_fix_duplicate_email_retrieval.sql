-- Fix duplicate email retrieval in RAG search
-- Migration: 20260619000000_fix_duplicate_email_retrieval
-- 
-- Problem: search_emails() was returning multiple chunks from the same email
-- Solution: Use DISTINCT ON (e.id) to return only the highest similarity chunk per email

-- Drop and recreate the function with deduplication
